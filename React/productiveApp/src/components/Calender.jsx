...
function Calendar() {
  ...
  useEffect(() => {
    ...
    const isCurrentMonth = date.getMonth() === adjustedMonth && date.getFullYear() === adjustedYear;
      ...
      <div className="w-full">
        ...
        <ul className="flex flex-wrap items-center gap-1 py-2 px-3">
          {days.map((day, index) => (
            <li 
              key={index} 
              className={`w-[50px] h-[50px] flex justify-center items-center border ${day.isToday ? 'bg-red-700 text-white' : ''}`}>
              {day.day !== 0 ? day.day : ''}
            </li>
          ))}
        </ul>
